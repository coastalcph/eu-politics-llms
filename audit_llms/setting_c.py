...
import tqdm
import os
from configure_prompt import TEMPLATE_EU_PARTY_GUESS
...
def main():
    ...
    for party_name in config.party_names:
        examples = []
        for statement_idx, example in tqdm.tqdm(enumerate(euandi_statements)):
            if party_positions[party_name][f'statement_{statement_idx+1}']['translated_position_google'] is not None:
                ...
                party_position = party_positions[party_name][f'statement_{statement_idx+1}']['translated_position_google']
                party_position = party_position.replace(party_name, 'party')
                question = TEMPLATE_EU_PARTY_GUESS.format(party_position,
                                                          config.party_names[0],
                                                          config.party_names[1],
                                                          config.party_names[2],
                                                          config.party_names[3],
                                                          config.party_names[4])
        ...
        for statement in examples:
            ...
            for response in statement['model_responses']:
                if response.lower().startswith('(a)'):
                    normalized_model_responses.append(config.party_names[0])
                elif response.lower().startswith('(b)'):
                    normalized_model_responses.append(config.party_names[1])
                elif response.lower().startswith('(c)'):
                    normalized_model_responses.append(config.party_names[2])
                elif response.startswith('(d)'):
                    normalized_model_responses.append(config.party_names[3])
                elif response.lower().startswith('(e)'):
                    normalized_model_responses.append(config.party_names[4])
                else:
        ...
        with open(os.path.join(DATA_DIR, "model_responses", f"{output_name}_{party_name.lower()}_setting_c_responses.jsonl"), "w") as f:
