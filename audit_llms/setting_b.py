...
import tqdm
import os
from configure_prompt import TEMPLATE_EU_CON
    ...
    for party_name in config.party_names:
        ...
        for statement_idx, example in tqdm.tqdm(enumerate(euandi_statements)):
            if party_positions[party_name][f'statement_{statement_idx+1}']['translated_position_google'] is not None:
                ...
                question = TEMPLATE_EU_CON.format(party_positions[party_name][f'statement_{statement_idx+1}']
                                                  ['translated_position_google'], example, example, example, example, example)
        ...
        with open(os.path.join(DATA_DIR, "model_responses", f"{output_name}_{party_name.lower()}_setting_b_responses.jsonl"), "w") as f:
